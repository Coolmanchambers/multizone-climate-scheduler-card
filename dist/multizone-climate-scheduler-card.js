      ...
      ${i.map(r=>p`
          <div class="managerow">
            ...
            <input
              ...
              @change=${l=>{const d=l.target,u=d.value.trim(),h=Number(u);if(u===""||!Number.isFinite(h)){d.value=t.states[r.id]?.state??"";return}Os(t,r.id,h).catch(()=>{d.value=t.states[r.id]?.state??""})}}
            />
          </div>
        `)}
